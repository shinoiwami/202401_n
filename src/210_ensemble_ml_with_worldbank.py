...
###
# GOAL: output[destination_index][country][source_index]["year"] = gap_year
#       output[destination_index][country][source_index]["importance"] = importance
...
#						print(zipname, data0.shape)
						...
						for cc in country_index:
							...
							for yr in reversed(range(year_min, year_max+1)):
								if nan == 0 and not pd.isnull(data0[str(yr)][country_num[cc]]) and not pd.isnull(data1[str(yr)][country_num[cc]]):
									year1 = yr
									nan = 1
								elif nan == 1 and (pd.isnull(data0[str(yr)][country_num[cc]]) or pd.isnull(data1[str(yr)][country_num[cc]])):
									year0 = yr+1
									nan = 2
									break
...
#							print(cc, year1, year0, gap_period, year1-year0, gap_period+1)
							if nan == 0 or year1-year0 <= gap_period+1:
								continue
...
#								print("Success:", importances, r2)

								i = 0
								importance_score = 0
								importance_gap = 0
								for im in importances:
									if importance_score < im:
										importance_score = im
										importance_gap = i
									i += 1
								output[dst][cc].setdefault(src, {})
								output[dst][cc][src]["year"] = year1 - importance_gap
								output[dst][cc][src]["importance"] = importance_score
